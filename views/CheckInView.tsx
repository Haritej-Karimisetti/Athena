...
export const CheckInView: React.FC<CheckInViewProps> = ({ onBack, onCheckInSuccess }) => {
  ...
  useEffect(() => {
    if (scanState.status !== 'SCANNING' || scannerRef.current?.getState() === Html5QrcodeScannerState.SCANNING) {
      return;
    }
    ...
    const onScanSuccess = async (decodedText: string) => {
      ...
      // Using the mock validation function
      const response = await mockCheckInValidation(decodedText);
